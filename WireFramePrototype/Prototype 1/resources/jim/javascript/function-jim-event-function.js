...
(function(window, undefined) {
  jQuery.extend(jimEvent.fn, {
    ...
    "jimRegExp": function(parameters, obj) {
      ...
      if(jimUtil.exists(parameters[0]) && jimUtil.exists(parameters[1])) {
        op = self.evaluateExpression(parameters[0], obj) + "";
        regexp = new RegExp(self.evaluateExpression(parameters[1], obj));
    	    ...
    	    switch(type) {
    	        ...
    	        case itemType.image:
    	        	...
    	        	if ($target.find("img").length > 0)
    	        	  	value = $target.find("img").attr("src");
    	        	else
    	        		value = $target.attr("systemName");
    	            break;
    ...
    "jimGetPosition": function($target,absolute) {
    	if(jimUtil.exists($target) && $target.length) {
			...
			if ($newTarget.is(".masterinstance, .group")) {
				var bounds = jimUtil.getRelativeItemBounds($newTarget);
				left = bounds.x;
				top = bounds.y;
				if (absolute) {
					var parent = $newTarget.parent().closest("#alignmentBox ,.firer:not(.screen, .template,.datarow, #simulation), .masterinstance>.offset");
					var parentPosition = jimEvent.fn.jimGetPosition(parent, true);
			...
			if($newTarget.hasClass("pin") && $newTarget.css("position")=="fixed"){
				var hPin = jimPin.getHorizontalPin($newTarget);
				var vPin = jimPin.getVerticalPin($newTarget);
				top = jimPin.calculateAbsoluteTop($newTarget,$newTarget.data("offsetY"),vPin,"absolute");
				left = jimPin.calculateAbsoluteLeft($newTarget,$newTarget.data("offsetX"),hPin,"absolute");
			}
			else{
				if(absolute){
					var parents = $newTarget.parents("#alignmentBox ,.firer:not(.screen, .template,.datarow, #simulation), .masterinstance>.offset");
    ...
    "jimGetWidth": function($targets) {
    	if(jimUtil.exists($targets) && $targets.length) {
    	    ...
    	    for(i=0, iLen=$targets.length; i<iLen; i+=1) {				
				...
				if ($currentTarget.is(".masterinstance, .group")) {
					width = jimUtil.getRelativeItemBounds($currentTarget).width;
				} else {
					if (!$currentTarget.is(":visible") && $currentTarget.hasClass("autofit") && $currentTarget.hasClass("singleline")) {
						$currentTarget.jimForceVisibility();
						width += $currentTarget.outerWidth();
						$currentTarget.jimUndoVisibility();
					}
					else
						width += $currentTarget.outerWidth();
    	        
					if ($currentTarget.jimGetType() == itemType.file)
    ...
    "jimGetHeight": function($target) {
    	if(jimUtil.exists($target) && $target.length) {
			if ($target.is(".masterinstance, .group")) {
				height = jimUtil.getRelativeItemBounds($target).height;
			} else {
				var height = 0;
				if (!$target.is(":visible") && $target.hasClass("autofit")) {
					$target.jimForceVisibility();
					height = $target.outerHeight();
					$target.jimUndoVisibility();
				} else height = $target.outerHeight();
			}
    		return height;
    	}
    },
